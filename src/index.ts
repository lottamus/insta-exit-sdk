...
import {
  ...
  DepositRequest,
  FetchOption,
  InternalBiconomyOption,
  ManualExitResponse,
  Options,
  PoolInformationResponse,
  SupportedToken,
  Transaction,
  TransactionResponse,
} from "./types";
...
class Hyphen {
  ...
  _init = async () => {
    if (this.provider) {
      ...
      if (networkId) {
        const supportedTokens = await this._getSupportedTokensFromServer(
          networkId
        );
  ...
  _validate = (options: Options) => {
    ...
    if (
      ...
      throw new Error(
        `apiKey is required under biconomy option. Either disable biconomy or provide apiKey`
      );
    }
  };
  ...
  getERC20TokenDecimals = (address: string) => {
    const tokenContract = new ethers.Contract(
      address,
      config.erc20TokenABI,
      this._getProvider(false)
    );
    if (tokenContract) {
      return tokenContract.decimals();
    } else {
      throw new Error(
        "Unable to create token contract object. Please check your network and token address"
      );
    }
  };
  ...
  preDepositStatus = (
    ...
    return new Promise(async (resolve, reject) => {
      ...
      fetch(
        `${self._getHyphenBaseURL()}${config.checkRequestStatusPath}`,
        fetchOptions
      )
  ...
  _getSupportedTokensFromServer = (
    networkId: number
  ): Promise<SupportedToken[] | void> => {
    const self = this;
    return new Promise(async (resolve, reject) => {
      const fetchOptions: FetchOption = this.getFetchOptions("GET");
      fetch(
        `${self._getHyphenBaseURL()}${
          config.getSupportedTokensPath
        }?networkId=${networkId}`,
        fetchOptions
      )
        .then((response) => response.json())
        .then((response) => {
          if (response && response.supportedPairList) {
            ...
            const error = self.formatMessage(
              RESPONSE_CODES.ERROR_RESPONSE,
              `Unable to get supported tokens`
            );
            self._logMessage(error);
  ...
  deposit = async (
    ...
    if (isNativeAddress(request.tokenAddress)) {
      const depositTransaction =
        await this._depositTokensToLiquidityPoolManager(request);
      if (depositTransaction) {
        this.listenForExitTransaction(
          depositTransaction,
          typeof request.fromChainId === "number"
            ? request.fromChainId
      ...
      this._logMessage(
        `Allowance given to LiquidityPoolManager is ${allowance}`
      );
  ...
  listenForExitTransaction = async (
    ...
    if (onFundsTransfered) {
      const interval =
        this.options.exitCheckInterval || config.defaultExitCheckInterval;
      await transaction.wait(1);
      this._logMessage(
        `Deposit transaction Confirmed. Listening for exit transaction now`
      );
      let invocationCount = 0;
      const intervalId = setInterval(async () => {
        const depositHash = transaction.hash;
        const response: any = await this.checkDepositStatus({
          depositHash,
          fromChainId,
        });
        invocationCount++;
        ...
        if (invocationCount >= config.maxDepositCheckCallbackCount) {
          this._logMessage(
            `Max callback count reached ${config.maxDepositCheckCallbackCount}. Clearing interval now`
          );
          clearInterval(this.depositTransactionListenerMap.get(depositHash));
          this.depositTransactionListenerMap.delete(depositHash);
        }
      ...
      this._logMessage(
        `onFundsTransfered method is missing from options so not listening for exit transaction`
      );
    }
  };
  ...
  checkDepositStatus = (
    ...
    return new Promise(async (resolve, reject) => {
      if (
        ...
        const fetchOptions: FetchOption = this.getFetchOptions("GET");
        const getURL = `${self._getHyphenBaseURL()}${
          config.checkTransferStatusPath
        }?depositHash=${depositRequest.depositHash}&fromChainId=${
          depositRequest.fromChainId
        }`;
  ...
  async getPoolInformation(
    ...
    fromChainId: number,
    toChainId: number
  ): Promise<PoolInformationResponse> {
    if (!tokenAddress && fromChainId === void 0 && toChainId === void 0) {
      throw new Error(
        this.formatMessage(
          RESPONSE_CODES.BAD_REQUEST,
          "Bad input params. fromChainId, toChainId and tokenAddress are mandatory parameters"
        ).message
      );
    }
    ...
    try {
      const response = await fetch(
        `${this._getHyphenBaseURL()}${
          config.getPoolInfoPath
        }?tokenAddress=${tokenAddress}&fromChainId=${fromChainId}&toChainId=${toChainId}`,
        this.getFetchOptions("GET")
      );

      return await response.json();
    } catch (error) {
      this._logMessage(error as Error);
      throw error;
    }
  }
  ...
  approveERC20 = async (
    ...
    if (currentNetwork) {
      ...
      if (!erc20ABI) {
        throw new Error(
          `ERC20 ABI not found for token address ${tokenAddress} on networkId ${currentNetwork.chainId}`
        );
      }
      ...
      const tokenInfo = config.tokenAddressMap[tokenAddress.toLowerCase()]
        ? config.tokenAddressMap[tokenAddress.toLowerCase()][
      ...
      if (tokenContract) {
        if (
          ...
          this._logMessage(
            `Infinite approval flag is true, so overwriting the amount with value ${amount}`
          );
        }
        if (spender && approvalAmount) {
          ...
          if (this.options.biconomy && this.options.biconomy.enable) {
            const customMetaTxSupport =
              config.customMetaTxnSupportedNetworksForERC20Tokens[
                currentNetwork.chainId as keyof typeof config.customMetaTxnSupportedNetworksForERC20Tokens
              ];
            if (
              customMetaTxSupport &&
              ...
              const functionSignature =
                tokenContractInterface.encodeFunctionData("approve", [
                  spender,
                  approvalAmount.toString(),
                ]);
              const tokenData = getMetaTxnCompatibleTokenData(
                tokenAddress,
                currentNetwork.chainId
              );
              const dataToSign = await getERC20ApproveDataToSign({
                ...
                salt:
                  "0x" + currentNetwork.chainId.toString(16).padStart(64, "0"),
              });
              ...
              if (_provider) {
                ...
                const { data } =
                  await tokenContract.populateTransaction.executeMetaTransaction(
                    userAddress,
                    functionSignature,
                    r,
                    s,
          ...
          this._logMessage(
            `One of the inputs is not valid => spender: ${spender}, amount: ${amount}`
          );
        }
      } else {
        this._logMessage("Token contract is not defined");
        throw new Error(
          "Token contract is not defined. Please check if token address is present on the current chain"
        );
      }
    } else {
      throw new Error(
        "Unable to get current network from provider during approveERC20 method"
      );
    }
  };
  ...
  triggerManualTransfer = (
    depositHash: string,
    chainId: string
  ): Promise<ManualExitResponse | undefined> => {
    const self = this;
    return new Promise((resolve, reject) => {
      if (depositHash && chainId && depositHash !== "" && chainId !== "") {
        ...
        reject(
          this.formatMessage(
            RESPONSE_CODES.BAD_REQUEST,
            "Bad input params. depositHash and chainId are mandatory parameters"
          )
        );
      }
    });
  ...
  _sendTransaction = async (_provider: any, rawTransaction: Transaction) => {
    if (_provider && rawTransaction) {
      const transactionHash = await _provider.send("eth_sendTransaction", [
        rawTransaction,
      ]);
      ...
      throw new Error(
        "Error while sending transaction. Either provider or rawTransaction is not defined"
      );
    }
  };
  ...
  _logMessage = (message: object | string) => {
    if (this.options && this.options.debug) console.log(message);
  };
}

export { Hyphen, config, RESPONSE_CODES, EXIT_STATUS, SIGNATURE_TYPES };
